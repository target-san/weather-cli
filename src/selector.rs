...
impl Selector {
    ...
    pub fn add_provider<T: Provider + 'static>(&mut self, name: impl Into<Cow<'static, str>>) {
        ...
        match self.providers.entry(name) {
            BTreeEntry::Vacant(e) => { e.insert(Box::new(ProviderFactoryT::<T>::new())); },
    ...
    pub fn create(&self, name: impl Into<Cow<'static, str>>, config: toml::Value) -> anyhow::Result<Box<dyn Provider>> {
        let name: Cow<'static, str> = name.into();
